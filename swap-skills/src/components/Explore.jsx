...
const SKILL_OPTIONS = Array.from(new Set(users.flatMap(u=>[...u.skillsHave, ...u.skillsWant]))).slice(0,30)


const filtered = users.filter(u => {
const q = query.trim().toLowerCase()
const byName = !q || u.name.toLowerCase().includes(q)
const bySkill = !skillFilter || u.skillsHave.includes(skillFilter) || u.skillsWant.includes(skillFilter)
...
<div className="badge bg-secondary mb-1">Has: {u.skillsHave.length}</div>
<div className="badge bg-success">Wants: {u.skillsWant.length}</div>
</div>
</div>
<div className="mb-2">
{u.skillsHave.map(s=> <span key={s} className="badge bg-light text-dark me-1">{s}</span>)}
</div>
